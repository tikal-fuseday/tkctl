...
resource "aws_subnet" "public" {
  ...
  tags = "${merge(var.tags, var.public_subnet_tags, map("Name", format("Public Subnet %s", element(var.azs, count.index))))}"
}
...
resource "aws_subnet" "private" {
  ...
  tags = "${merge(var.tags, var.private_subnet_tags, map("Name", format("Private Subnet %s", element(var.azs, count.index))))}"
}
...
resource "aws_subnet" "database" {
  ...
  tags = "${merge(var.tags, var.database_subnet_tags, map("Name", format("DB Subnet %s", element(var.azs_db, count.index))))}"
}
...
resource "aws_subnet" "redshift" {
  ...
  tags = "${merge(var.tags, var.redshift_subnet_tags, map("Name", format("Redshift Subnet %s", element(var.azs, count.index))))}"
}
...
resource "aws_subnet" "elasticache" {
  ...
  tags = "${merge(var.tags, var.elasticache_subnet_tags, map("Name", format("Elasticache Subnet %s", element(var.azs, count.index))))}"
}
...
resource "aws_eip" "nat" {
  count = "${(var.enable_nat_gateway && !var.reuse_nat_ips) ? (var.single_nat_gateway ? 1 : length(var.azs_nat)) : 0}"

  vpc = true

  tags = "${merge(var.tags, map("Name", format("%s-%s", var.name, element(var.azs_nat, (var.single_nat_gateway ? 0 : count.index)))))}"
}

resource "aws_nat_gateway" "this" {
  count = "${var.enable_nat_gateway ? (var.single_nat_gateway ? 1 : length(var.azs_nat)) : 0}"
...
########################
#Office Route for VPC
########################
resource "aws_route" "office_public" {
...
resource "aws_route" "office_private" {
  count = "${length(var.office_subnets)}"
...
#Sit2Site VPN
###########################
#resource "aws_route" "site2sita_public" {
#  count = "${length(var.vpc_subnets)}"

#  route_table_id         = "${aws_route_table.public.id}"
#  destination_cidr_block = "${var.vpc_subnets[count.index]}"
#  instance_id            = "${var.instance_gateway}"
#}

#resource "aws_route" "site2site_private" {
#  count = "${length(var.vpc_subnets)}"
